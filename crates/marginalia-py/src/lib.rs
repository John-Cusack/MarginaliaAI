...
/// Unicode version every character table behind these seams implements
/// (NFKC, `\w`, whitespace), exposed as `marginalia_rs.UNICODE_VERSION`.
/// Python's answers follow `unicodedata.unidata_version`, so the Python side
/// only routes `auto` here when the two agree.
const UNICODE_VERSION: &str = marginalia_text::UNICODE_VERSION;
...
mod tests {
    ...
    fn module_tree_registers_text_with_version() {
        ...
        Python::with_gil(|py| {
            ...
            assert_eq!(public("parse"), ["parse_markdown"]);
            assert!(
                !root.hasattr("works").unwrap(),
                "works was cut back to Python"
            );
        });
    }
