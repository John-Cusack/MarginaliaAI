...
/// Collapse whitespace runs only — the narrower re-anchoring transform.
pub fn normalize_whitespace(text: &str) -> String {
    strip(&WHITESPACE_RUNS.replace_all(text, " ")).to_owned()
}
...
fn linebreak_hyphen_deletions(text: &str) -> HashSet<usize> {
    ...
    for m in linebreak_matches(text) {
        let g1_end = char_idx_at(text, m.g1_end_b);
        let g2_start = char_idx_at(text, m.g2_start_b);
        deleted.extend(g1_end..g2_start);
    }
    deleted
}

/// Character index of a byte offset (regex reports bytes; text uses chars).
fn char_idx_at(text: &str, byte: usize) -> usize {
