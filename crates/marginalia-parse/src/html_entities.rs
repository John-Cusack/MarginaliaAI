...
/// `HTML_ENTITIES`: sorted `(name, replacement)` entries.
...
/// `HTML5_ENTITIES`: sorted `(name, replacement)` entries.
pub const HTML5_ENTITIES: &[(&str, &str)] = &[
    ("AElig", "\u{00C6}"),
    ("AElig;", "\u{00C6}"),
    ("AMP", "&"),
    ("AMP;", "&"),
...
/// Binary lookup over a sorted entity table.
...
/// A bare-name lookup, exactly what the builder consults for text.
pub fn html_entity(name: &str) -> Option<&'static str> {
    lookup(HTML_ENTITIES, name)
}

/// An exact-key lookup (`html.entities.html5`), what attributes use.
pub fn html5_entity(key: &str) -> Option<&'static str> {
    lookup(HTML5_ENTITIES, key)
}
