...
class TestStructuralParity:
    ...
    async def test_non_json_section_values_rejected_on_rust(self, monkeypatch):
        pytest.importorskip("marginalia_rs")
        chunker = StructuralChunker()
        # Offsets present, so the Python path locates and echoes the UUID;
        # the Rust path fails the adapter's strict `json.dumps`.
        sections = [{"text": "Hi there.", "page": uuid.uuid4(), "char_start": 0, "char_end": 9}]
        monkeypatch.setenv("RE_RUST_BACKEND", "rust")
        with pytest.raises(TypeError):
            await chunker.chunk(sections)
        monkeypatch.setenv("RE_RUST_BACKEND", "python")
        drafts = await chunker.chunk(sections)
        assert drafts[0].locator["page"] == sections[0]["page"]
