...
``RE_RUST_BACKEND``: ``auto`` (default: Rust when the accelerator is installed
*and* was built for this interpreter's Unicode version, else Python), ``rust``
(force; missing accelerator raises loudly), ``python`` (force the pure-Python
path; the bisection/rollback switch).

The Unicode check is what makes "byte-identical" true. The accelerator's NFKC
...
16.0). ``auto`` only takes the Rust path when ``marginalia_rs.UNICODE_VERSION``
equals ``unicodedata.unidata_version``; anywhere else, and with any wheel old
enough to lack the constant, it stays on Python. ``rust`` skips the check so
parity suites can still force the comparison.
...
def backend() -> str:
    ...
    if mode == "auto":
        try:
            accelerator = __import__("marginalia_rs")
        except ImportError:
            return "python"
        if getattr(accelerator, "UNICODE_VERSION", None) != unicodedata.unidata_version:
            return "python"
        return "rust"
    raise ValueError(
