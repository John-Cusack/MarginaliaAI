...
class StructuralChunker:
    ...
    async def chunk(
        ...
        if rs is not None:
            # Python is the reference. Sections the Rust path refuses (lone
            # surrogates, NaN, offsets past i64, non-dict entries, fractional
            # offsets) get Python's own answer: its output or its error.
            try:
