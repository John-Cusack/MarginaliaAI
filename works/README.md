...
2. **Every citation is a front-matter entry**, not prose. An entry names the
   ...
   citations:
     - id: c1                    # referenced in the body as [^c1]
       ...
       edition_key: SIDER_2019   # the join once P3 lands; fill it from day one
...
4. **Bibliographic display is derived, never hand-typed.** Author, title, year
   ...
   identity (`edition_key`) and location (`edition`, `locator`). An `edition_key`
...
- **A clean upgrade path.** Front-matter `edition_key`/`edition` are exactly the
